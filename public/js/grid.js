...
var Grid = (function() {
	...
	function initEvents() {
		...
		// when clicking an item, show the preview with the item´s info and small image.
	...
	Preview.prototype = {
		create : function() {
			...
			this.$previewInner = $( '<div class="og-expander-inner"></div>' ).append( this.$button, this.$fullimage, this.$details, this.$previewInner2 );
		...
		update : function( $item ) {
			...
			if( typeof self.$smallImg != 'undefined' ) {
				self.$smallImg.remove();
			}

			// preload small image and add it to the preview
			// for smaller screens we don´t display the small image (the media query will hide the fullimage wrapper)
			if( self.$fullimage.is( ':visible' ) ) {
				...
				$( '<img/>' ).load( function() {
					...
					if( $img.attr( 'src' ) === self.$item.children('a').data( 'smallsrc' ) ) {
						...
						self.$smallImg = $img.fadeIn( 350 );
						self.$fullimage.append( self.$smallImg );
					}
				} ).attr( 'src', eldata.smallsrc );	
			}
		...
		close : function() {
			...
			setTimeout( $.proxy( function() {
				...
				if( typeof this.$smallImg !== 'undefined' ) {
					this.$smallImg.fadeOut( 'fast' );
				}
